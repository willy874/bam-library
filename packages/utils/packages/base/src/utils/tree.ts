...
export function findNode<T = any>(tree: any, func: AnyFunction, config: Partial<TreeHelperConfig> = {}): T | null {
  config = getConfig(config);
  const { children } = config;
  const list = [...tree];
  for (const node of list) {
...
export function findNodeAll<T = any>(tree: any, func: AnyFunction, config: Partial<TreeHelperConfig> = {}): T[] {
  config = getConfig(config);
  const { children } = config;
  const list = [...tree];
  const result: T[] = [];
...
export async function forEach<T = any>(tree: T[], func: (n: T) => any, config: Partial<TreeHelperConfig> = {}) {
  ...
  for (let i = 0; i < list.length; i++) {
    const result = await func(list[i]);
 ...
 */
export function treeMap<T = any>(treeData: T[], opt: { children?: string; conversion: AnyFunction }): T[] {
